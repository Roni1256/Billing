  ...
  return (
    ...
    <nav className={`w-full  flex items-center justify-between lg:px-32 px-5 py-7  md:flex-row gap-5  ${!isAuth?"dark:bg-transparent ":"dark:bg-black/40 hidden"} `}>
        ...
        <div className="w-full flex  justify-end   gap-8">
            ...
            {isAuth&&<Link to={"/dashboard"} className=' font-bold hover:underline'>Dashboard</Link>}
        
        </div>
        <div className="flex w-full lg:flex items-center justify-end gap-10">
           { !isAuth && <Button label={"Get Started"} click={() => {navigate("/auth")}}/>}
            { isAuth &&
                <button className=' p-2 rounded-full ring-2 ring-slate-400 ' onClick={handlelogout}>
                    <BiLogOut size={20}/>
                </button>
            }
        </div>
        <button className=' p-2 rounded-full ring-2 ring-slate-400 ' onClick={handleDark}>
            {isDark?
