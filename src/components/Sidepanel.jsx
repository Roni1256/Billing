...
import { TbListDetails } from "react-icons/tb";
  const Sidepanel = ({ openpanel,toggle }) => {
    ...
    return (
        ...
        <div className={`z-10 bg-transparent md:bg-white    dark:bg-black/40 backdrop-blur-md h-screen w-[300px] md:relative absolute left-0 dark:text-white p-4 pt-7 border-r-2 border-gray-700/20 transition-all duration-300 ${openpanel ? "left-0" : "left-[-400px]"} `}>
          <div className="mb-8">
            ...
            <ul className="space-y-4">
              {
                links.map((link)=>(
                  <li key={link.name}>
                    <Link to={link.link} className={`flex items-center p-3 hover:bg-gray-700 focus:bg-gray-700 hover:text-white rounded-lg transition-colors  active:bg-gray-700 ${link.link === window.location.pathname ? "bg-gray-700 text-white" : "dark:text-gray-200"}`} onClick={() => window.innerWidth <= 789 && toggle()}>
                      {link.icon}
                      <span>{link.name}</span>
                    </Link>
                  </li>
