                ...
                <ul className="space-y-3 sm:space-y-4">
                  {
                    links.map((link)=>(
                      <li key={link.name}>
                        <Link to={link.link} className={`flex items-center p-2 sm:p-3 text-sm sm:text-base hover:bg-gray-700 focus:bg-gray-700 hover:text-white rounded-lg transition-colors active:bg-gray-700 ${link.link === window.location.pathname ? "bg-gray-700 text-white" : "dark:text-gray-200"}`} onClick={() => window.innerWidth <= 789 && toggle()}>
                          {link.icon}
