...
const Admin = () => {
  ...
  const deleteAllUser = async () => {
    await axios
      .delete(API_URLS.deleteAllUsers)
      .then((res) => {
  ...
  const deleteUser = async (id) => {
    console.log(id);
    await axios
      .delete(API_URLS.admindelete + id)
      .then((res) => {
        console.log(res.data);
      })
      .catch((err) => {
  ...
  if (isAdmin) {
    return (
      <div className="bg-gray-100 p-10 flex flex-col items-center gap-5 absolute top-0 left-0 w-full  justify-center">
        <h1 className="text-slate-900 text-2xl font-bold w-full text-left">
          Admin Dashboard
        </h1>
        <Element
          label="Delete all the Users"
          btnLabel="Delete"
        ...
        {currentUserView && (
          <div className="h-screen text-slate-800 w-full flex justify-center absolute top-0 left-0 items-center p-10 backdrop-blur-md bg-slate-800/20">
            <div className="bg-white w-full max-w-[600px] rounded-md ">
              <h1 className="text-3xl font-bold flex w-full justify-between p-5">
                User Details{" "}
                <button
              ...
              <div className="w-full h-[400px] overflow-auto scrollbar-none">
                {Object.entries(currentView).map(([key, value]) => (
                  <>
                    <div
                      key={key}
                      className="flex max-w-[600px] w-full bg-white p-5 rounded-lg items-center justify-between gap-10 h-fit"
                    >
...
function Element({ label, btnLabel, action, btnStyle, msg, msgcolor, click }) {
  return (
    <>
      <div
        className="max-w-[900px] w-full flex items-center justify-between p-3 bg-white text-slate-800 font-semibold rounded-lg ring-1 ring-slate-500/40 shadow-md cursor-pointer"
        onClick={click ? click : () => {}}
      >
        <h1>{label}</h1>
        {msg && (
          <span className={`text-slate-700 font-bold ${msgcolor}`}>{msg}</span>
        )}
        {btnLabel && (
          <Button label={btnLabel} click={action} style={btnStyle} />
        )}
      </div>
    </>
  );
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead>
                <tr>
                  <th
                    scope="col"
                    class="px-6 py-3 text-start text-xs font-medium text-slate-900 uppercase"
                  >
                    Name
                  </th>
                  <th
                  ...
                  <th
                    scope="col"
                    class="px-6 py-3 text-end text-xs font-medium text-slate-900 uppercase"
                  >
                    Delete
                  </th>
                </tr>
