...
const Customers = ({ data, isLoading, updates }) => {
  ...
  return (
    <div className="relative p-3">
              ...
              {data.customers.map((customer) =>{ 
                ...
                return(
                  <div className="mb-3 md:mb-0">
                    <div className="block md:hidden text-xs font-semibold mb-1">Customer Information</div>
                    <CustomerCard
                      ...
                      time={customer.createdAt?customer.createdAt.slice(11,16):"N/A"}
      ...
      {showCurrent && <Details data={currentCustomer} closeDetails={()=>setCurrent(false)}/>}
      </main>
    </div>
  );
};
...
function CustomerCard({ name, email, phone, address, date,style,click,time }) {
  return (
    <>
      <div className={`flex flex-col md:flex-row justify-between p-3 md:p-4 font-semibold gap-2 md:gap-8 text-xs md:text-sm h-fit rounded-lg ring-1 ring-slate-500 ${style}`} onClick={click}>
        ...
        <h1 className="hidden md:block text-md font-bold w-full text-center">{name}</h1>
  ...
  return <>
    <div className="h-full absolute w-full top-0 -left-0 backdrop-blur-md flex items-center justify-center p-2 md:p-4">
      <div className="w-full max-w-[600px] h-fit bg-white rounded-lg shadow-sm p-3 md:p-8 text-sm md:text-base">
        <h1 className="text-lg md:text-2xl font-medium mb-3 md:mb-6 text-gray-800">{data?.email || "N/A"}</h1>
        ...
        <ul className="space-y-3 md:space-y-6">
          ...
          <li className="text-gray-700">Date: {data?.createdAt?.slice(0,10) || "N/A"}</li>
          <li className="text-gray-700">Time: {data?.createdAt?.slice(11,16) || "N/A"}</li>
          <li className="text-gray-700">Invoice Number: {data?.invoice_number || "N/A"}</li>
 
          <div className="overflow-x-auto">
            <table className="w-full mt-3 md:mt-6 text-xs md:text-base">
              <tr className="border-b">
                <th className="py-2 md:py-3 text-left text-gray-600 font-bold">Product</th>
                <th className="py-2 md:py-3 text-left text-gray-600 font-bold">Price</th>
                <th className="py-2 md:py-3 text-left text-gray-600 font-bold">Quantity</th>
                <th className="py-2 md:py-3 text-left text-gray-600 font-bold">Subtotal</th>
              </tr>
              {data?.products?.map((item) => (
                <tr className="border-b">
                  <td className="py-2 md:py-3 text-gray-700">{item?.name || "N/A"}</td>
                  <td className="py-2 md:py-3 text-gray-700">{item?.price || "N/A"}</td>
                  <td className="py-2 md:py-3 text-gray-700">{item?.userquantity || "N/A"}</td>
                  <td className="py-2 md:py-3 text-gray-700">{item?.price * item?.userquantity || "N/A"}</td>
                </tr>
              ))}
            </table>
          </div>
          <li className="text-gray-700">Total Items: {data?.totalItems || "N/A"}</li>
          <li className="text-gray-700">Total Price: {data?.totalPrice || "N/A"}</li>
        </ul>
        <Button label={"Close"} style={"bg-red-600 hover:bg-red-700 mt-3 md:mt-5 w-full md:w-auto"} click={closeDetails} />
      </div>
    </div>  
  </>
}