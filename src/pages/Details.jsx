...
const Details = ({ data, isLoading, updates }) => {
  ...
  const update = async () => {
    setPatchLoad(true);
    await axios
      .patch(API_URLS.updatecompany + data._id, updateData)
      .then((res) => {
  ...
  return (
    <div className="w-full rounded-lg backdrop-blur-md my-5 p-3 md:p-7 flex flex-col items-center">
      <h1 className="text-center text-2xl md:text-3xl font-bold text-gray-700 mb-2 dark:text-slate-300">
      ...
      {patchLoad && (
        <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md z-50">
          <Floats
            title={"Loading"}
            icon={<BiLoader size={40} className="animate-spin" />}
      ...
      {isSuccess && (
        <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md z-50">
          <Floats
            ...
            icon={<BiChevronRightCircle size={40} />}
            className="absolute top-0 right-0"
          />
        </div>
      )}
